...
app = Flask(__name__)
app.secret_key = "expresphone-secret-2026"
...
GHANA_TOKEN = ""
...
def detect_country_by_phone_id(phone_number_id):
    ...
    print("INCOMING PHONE ID:", phone_number_id)

    for key, cfg in WHATSAPP_NUMBERS.items():

        cfg_phone = str(cfg.get("phone_id", "")).strip()

        print("CHECKING COUNTRY:", key, cfg_phone)

        if cfg_phone == phone_number_id:
            print("MATCH COUNTRY:", key)
            return key

    if phone_number_id == "1166801819841758":
        print("FORCED GHANA MATCH")
        return "ghana"

    print("NO MATCH -> DEFAULT ISRAEL")
    return "israel"
...
def mark_auto_reply_sent(wa_id):
    con = db()
    c = con.cursor()
...
def send_welcome_package(to, country):

    print("AUTO WELCOME START:", to, country)
    ...
    welcome_text = (
        ...
        "Here are some examples of our work and voice introduction 🙏"
    )

    send_message(to, welcome_text, country=country)

    # ===== SEND IMAGES =====
    for img in WELCOME_IMAGES:

        time.sleep(6)

        path = os.path.join(BASE_MEDIA_PATH, img)

        if not os.path.exists(path):
            print("WELCOME IMAGE NOT FOUND:", path)
            continue
        ...
        print("WELCOME IMAGE MEDIA ID:", img, media_id)
        ...
        print("WELCOME SEND IMAGE:", country, img, r.status_code, r.text[:1000])

        if r.status_code in (200, 201):
            save_message(to, "אני", "out", "image", "📷 תמונה", img, country)

    # ===== SEND AUDIOS =====
    for audio in WELCOME_AUDIOS:

        time.sleep(6)

        path = os.path.join(BASE_MEDIA_PATH, audio)

        if not os.path.exists(path):
            print("WELCOME AUDIO NOT FOUND:", path)
            continue
        ...
        print("WELCOME AUDIO MEDIA ID:", audio, media_id)
        ...
        r = requests.post(url, headers=headers, json=payload, timeout=30)
        print("WELCOME SEND AUDIO:", country, audio, r.status_code, r.text[:1000])

        if r.status_code in (200, 201):
            save_message(to, "אני", "out", "audio", "🎤 הודעה קולית", audio, country)

    print("AUTO WELCOME DONE")
...
def download_whatsapp_media(media_id, fallback_ext, country=DEFAULT_COUNTRY):
...
def send_message(to, text, save_out=True, country=DEFAULT_COUNTRY):
    ...
    phone_id = str(cfg["phone_id"]).strip()
    ...
    print("SEND RESPONSE:", r.text[:2000])

    if save_out and r.status_code in (200, 201):
...
def webhook():
    ...
    try:
        ...
        if msg_type == "text":
            ...
            media_file = download_whatsapp_media(media_id, ".ogg", country) if media_id else ""
            text = "🎤 הודעה קולית"

        elif msg_type == "image":
            media_id = msg.get("image", {}).get("id")
            media_file = download_whatsapp_media(media_id, ".jpg", country) if media_id else ""
            text = msg.get("image", {}).get("caption", "") or "🖼️ תמונה"

        elif msg_type == "video":
            media_id = msg.get("video", {}).get("id")
            media_file = download_whatsapp_media(media_id, ".mp4", country) if media_id else ""
            text = msg.get("video", {}).get("caption", "") or "🎥 וידאו"

        elif msg_type == "document":
            media_id = msg.get("document", {}).get("id")
            media_file = download_whatsapp_media(media_id, ".bin", country) if media_id else ""
        ...
        # ===== תגובה אוטומטית פעם אחת לכל מספר =====
        if should_auto_reply(sender):
            send_welcome_package(sender, country)
            mark_auto_reply_sent(sender)
...
@app.route("/reset_welcome/<wa_id>")
def reset_welcome(wa_id):
    if not session.get("ok"):
        return redirect("/login")

    con = db()
    c = con.cursor()
    c.execute("UPDATE chat_contacts SET last_auto_reply=NULL WHERE wa_id=?", (wa_id,))
    con.commit()
    con.close()
...
# SEND FROM PANEL
# =========================================================
@app.route("/send_welcome_now/<wa_id>")
def send_welcome_now(wa_id):
    if not session.get("ok"):
        return redirect("/login")

    country = get_contact_country(wa_id)
    send_welcome_package(wa_id, country)
    mark_auto_reply_sent(wa_id)

    return redirect(f"/?country={country}&chat={wa_id}")
    ...
    <div class="chat">
        <div class="chat-head">
            {% if selected %}
                ...
                <br>
                <a href="/send_welcome_now/{{selected}}" style="font-size:16px;color:#25d366;text-decoration:none">📦 שלח פתיחה עכשיו</a>
                &nbsp;|&nbsp;
                <a href="/reset_welcome/{{selected}}" style="font-size:16px;color:#ffcc00;text-decoration:none">🔁 אפס פתיחה</a>
            {% else %}
                בחר שיחה
            {% endif %}
        </div>
