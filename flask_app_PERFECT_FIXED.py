...
try:
    c.execute("ALTER TABLE chat_contacts ADD COLUMN internal_note TEXT")
    con.commit()
except:
    pass
...
def upload_whatsapp_media(file_path, mime_type="application/octet-stream"):
    ...
    print("UPLOAD MEDIA:", r.status_code, r.text[:1000])
    try:
...
def send_audio_message(to, media_id, save_out=True, local_filename="", mime_type="application/octet-stream"):
    url = f"https://graph.facebook.com/v18.0/{PHONE_ID}/messages"
    headers = {
        "Authorization": f"Bearer {TOKEN}",
        "Content-Type": "application/json"
    ...
    r = requests.post(url, headers=headers, json=payload, timeout=30)
    print("SEND MEDIA:", r.status_code, r.text[:1000])

    if save_out:
        save_message(to, "אני", "out", save_type, save_text, local_filename or "")
...
def send_audio_panel():
    ...
    if to and upload and upload.filename:
        ...
        if media_id:
            send_audio_message(to, media_id, save_out=True, local_filename=filename, mime_type=mime_type)
    ...
    try:
        if media_file:
            ...
            if os.path.exists(local_path):
                ...
                media_id = upload_whatsapp_media(local_path, mime_type)
            ...
            send_message(forward_to, text_to_forward.split("|||")[0])
...
function unlockAudio(){
    ...
    .catch((e)=>{
        ...
        alert("הדפדפן עדיין חסם צליל. לחץ שוב בתוך הדף ואז נסה.");
    });
}
...
function playNotifySound(){
    try{
        ...
        if(p !== undefined){
            p.catch(e=>console.log("play failed", e));
        }
    }catch(e){
        console.log(e);
    }
